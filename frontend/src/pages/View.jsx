...
console.log('seried data', seriesData)
  useEffect(() => {
    const fetchSeriesData = async () => {
      setLoading(true)
      try {
        const response = await axios.get(`https://3000-baitech365-aividedit-q7iuauhiu1c.ws-us110.gitpod.io/series_info?email=${userPlan?.email}`);
  ...
  const handleConnectYoutube = (item) => { 
    ...
    window.location.href = 'https://3000-baitech365-aividedit-q7iuauhiu1c.ws-us110.gitpod.io/connect_youtube'
   }
   ...
   useEffect(() => {
    ...
    const updateGoogleIdInDB = async () => {
      ...
      if (googleIdFromUrl && taskIdFromStorage) {
        try {
            const response = await axios.patch(`https://3000-baitech365-aividedit-q7iuauhiu1c.ws-us110.gitpod.io/googleId?taskId=${taskIdFromStorage}`, {
  ...
  const handleGenerateVideo = async (item) => {
    ...
    try {
      const response = await axios.post(`https://3000-baitech365-aividedit-q7iuauhiu1c.ws-us110.gitpod.io/generate_video`, {
  ...
  const handlePostVideo = async (item) => {
    try {
      const response = await axios.post(`https://3000-baitech365-aividedit-q7iuauhiu1c.ws-us110.gitpod.io/upload_video`, {
        email: userPlan?.email,
       });
      console.log(response?.data);
    } catch (error) {
